...
export async function getWeeklyTransactionsSummary(startDate, endDate) {
  ...
  try {
    ...
    const [variableExpenses, fixedExpenses, incomeTransactions, transfers, savings] = await Promise.all([
    ...
    // Aggregate all expense transactions
    let allTransactions = [];
    let totalExpenses = 0;
    const categories = {
      'Survival': 0,
    ...
    if (fixedExpenses.length > 0) {
      fixedExpenses.forEach(transaction => {
        ...
        // Fixed expenses typically count as "Survival" category
        categories['Survival'] += transaction.amount;
      });
    }
    
    // Note: We don't include income, transfers, or savings in total expenses
    
    // Sort all transactions by amount (highest first) for top items
    const topItems = allTransactions
    ...
    console.log(`Weekly summary: ${totalExpenses} total expenses, ${allTransactions.length} transactions`);
