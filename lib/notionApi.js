  ...
  try {
    ...
    if (response.ok) {
      ...
      const accounts = result.results.map(page => {
        // Get account name from title property
        const accountName = page.properties.Account?.title?.[0]?.text?.content || 'Unknown Account';
        
        // Get balance from number property (try common property names)
        let balance = 0;
        if (page.properties.Balance?.number !== undefined) {
          balance = page.properties.Balance.number;
        } else if (page.properties.Amount?.number !== undefined) {
          balance = page.properties.Amount.number;
        } else if (page.properties.Total?.number !== undefined) {
          balance = page.properties.Total.number;
        }
