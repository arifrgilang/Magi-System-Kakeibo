...
export async function getWeeklyTransactionsSummary(startDate, endDate) {
  console.log(`Fetching weekly summary from ${startDate} to ${endDate}`);
  
  try {
    // Query all transaction types for the date range
    const [variableExpensesList, fixedExpensesList, incomeTransactions, transfers, savings] = await Promise.all([
      getVariableExpensesInDateRange(startDate, endDate),
      getFixedExpensesInDateRange(startDate, endDate),
      getIncomeInDateRange(startDate, endDate),
      getTransfersInDateRange(startDate, endDate),
      getSavingsInDateRange(startDate, endDate)
    ]);
    ...
    variableExpensesList.forEach(transaction => {
      variableExpenses += transaction.amount;
      
      if (transaction.category && categories.hasOwnProperty(transaction.category)) {
        categories[transaction.category] += transaction.amount;
      }
    });
    
    // Process fixed expenses
    fixedExpensesList.forEach(transaction => {
      fixedExpenses += transaction.amount;
    });
    
    // Calculate total expenses (variable + fixed + transfers + savings)
    const transfersTotal = transfers.reduce((sum, t) => sum + t.amount, 0);
    const savingsTotal = savings.reduce((sum, s) => sum + s.amount, 0);
    totalExpenses = variableExpenses + fixedExpenses + transfersTotal + savingsTotal;
    
    // Get top 3 items for each expense type
    const topVariableItems = variableExpensesList
    ...
    const topFixedItems = fixedExpensesList
      .sort((a, b) => b.amount - a.amount)
      .slice(0, 3)
      .map(transaction => ({
        description: transaction.description,
    ...
    console.log(`Weekly summary: ${totalExpenses} total (${variableExpenses} variable + ${fixedExpenses} fixed)`);
    ...
    return {
      ...
      data: {
        ...
        topVariableItems,
        topFixedItems,
        transactionCount: variableExpensesList.length + fixedExpensesList.length
      }
    };
...
// Helper function to query variable expenses in date range
async function getVariableExpensesInDateRange(startDate, endDate) {
      ...
      if (response.ok) {
        ...
        for (const page of result.results) {
          const transaction = {
            ...
            category: await getRelationName(page.properties.Category?.relation?.[0]?.id)
          };
...
// Helper function to query fixed expenses in date range
async function getFixedExpensesInDateRange(startDate, endDate) {
...
// Helper function to query income in date range  
async function getIncomeInDateRange(startDate, endDate) {
  if (!Income_OkaneH_Transactions) return [];
  
  try {
    const response = await fetch(`https://api.notion.com/v1/databases/${Income_OkaneH_Transactions}/query`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${NOTION_TOKEN}`,
        'Content-Type': 'application/json',
...
// Helper function to query transfers in date range
async function getTransfersInDateRange(startDate, endDate) {
  if (!Accounts_OkaneH_Transfer) return [];
  
  try {
    const response = await fetch(`https://api.notion.com/v1/databases/${Accounts_OkaneH_Transfer}/query`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${NOTION_TOKEN}`,
        'Content-Type': 'application/json',
    ...
    if (response.ok) {
      ...
      for (const page of result.results) {
        const transaction = {
          type: 'transfer',
          description: page.properties.Transactions?.title?.[0]?.text?.content || 'No description',
          amount: page.properties['Total Amount (Out)']?.number || page.properties['Amount (Out)']?.number || 0,
          date: page.properties.Date?.date?.start || null
        };
...
// Helper function to query savings in date range
async function getSavingsInDateRange(startDate, endDate) {
  if (!Savings_OkaneH_Transactions) return [];
  
  try {
    const response = await fetch(`https://api.notion.com/v1/databases/${Savings_OkaneH_Transactions}/query`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${NOTION_TOKEN}`,
        'Content-Type': 'application/json',
    ...
    console.error('Error querying savings:', error);
  }
