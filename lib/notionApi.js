...
export async function addTransferToNotion(transferData) {
  ...
  try {
    ...
    // Calculate total amount (amount + admin fee)
    const totalAmount = transferData.amount + (transferData.adminFee || 0);
      ...
      properties: {
        ...
        'Type': {
          rich_text: [
            {
              text: {
                content: transferData.transferType,
              },
            },
          ],
        },
    ...
    if (transferData.adminFee && transferData.adminFee > 0) {
      payload.properties['Admin or Transaction Fee'] = {
        number: transferData.adminFee
      };
    }
