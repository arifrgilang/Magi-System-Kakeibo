...
// TEMPORARY DEBUG FUNCTION - Remove after fixing the property names
...
export async function addBasicFixedExpenseToNotion(expenseData) {
  ...
  await debugDatabaseProperties(Basic_Fixed_Expenses_OkaneH_Transactions);
  ...
  try {
    ...
    // Add relations if databases exist - using exact column names from your database
    ...
    if (itemId && Basic_Fixed_Expenses_OkaneH_Items) {
      payload.properties['Basic Fixed Expenses Items'] = {
        relation: [{ id: itemId }]
      };
    }
