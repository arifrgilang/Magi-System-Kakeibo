// lib/messageHandlers.js - Updated Message Handler Functions with Recurring Transactions
import { CONFIG } from './config.js';
import { SessionManager } from './sessionManager.js';
import { sendTelegramMessage, answerCallbackQuery } from './telegramApi.js';
import { showAccountBalances } from './balanceHandlers.js';
import { handleWeeklySummaryAction } from './weeklySummaryHandlers.js';
import { 
  showMainMenu, 
  showMonthSelection, 
...
import {
  showRecurringTransactionsMenu,
  handleRecurringTransactionSelection,
  handleRecurringMonthSelection,
  handleRecurringAccountSelection,
  handleRecurringUserInput
} from './recurringTransactionsHandlers.js';
...
export async function handleCallbackQuery(callbackQuery) {
  ...
  console.log('Parsed callback:', { action, params });
  ...
  switch (action) {
    ...
    case 'recent':
      ...
      break;
    case 'recurring':
      await handleRecurringAction(chatId, userId, params);
      break;
    case 'weekly':
      await handleWeeklySummaryAction(chatId, params);
      break;
    case 'show':
      if (params[0] === 'balances') {
        await showAccountBalances(chatId);
      }
      break;
    case 'back':
...
async function handleRecurringAction(chatId, userId, params) {
  const [subAction, ...rest] = params;
  
  console.log('handleRecurringAction called with:', { subAction, rest, fullParams: params });
  
  switch (subAction) {
    case 'menu':
      await showRecurringTransactionsMenu(chatId);
      break;
    case 'select':
      // Join the rest of the params to handle underscores in transaction keys
      const transactionKey = rest.join('_');
      console.log('Selecting recurring transaction:', transactionKey);
      await handleRecurringTransactionSelection(chatId, userId, transactionKey);
      break;
    case 'month':
      // Handle month selection: recurring_month_Lunch_Office_January
      // So rest = ['Lunch', 'Office', 'January']
      const month = rest[rest.length - 1]; // Last element is the month
      const transactionNameParts = rest.slice(0, -1); // Everything except the last element
      const transactionName = transactionNameParts.join('_');
      console.log('Month selection:', { transactionName, month });
      await handleRecurringMonthSelection(chatId, userId, transactionName, month);
      break;
    case 'account':
      // Handle account selection: recurring_account_Lunch_Office_January_BCA
      // So rest = ['Lunch', 'Office', 'January', 'BCA']
      const account = rest[rest.length - 1]; // Last element is the account
      const selectedMonth = rest[rest.length - 2]; // Second to last is the month
      const transactionNameForAccount = rest.slice(0, -2).join('_'); // Everything except last 2
      console.log('Account selection:', { transactionName: transactionNameForAccount, month: selectedMonth, account });
      await handleRecurringAccountSelection(chatId, userId, transactionNameForAccount, selectedMonth, account);
      break;
    case 'back':
      if (rest[0] === 'menu') {
        await showRecurringTransactionsMenu(chatId);
      } else if (rest[0] === 'month') {
        await showRecurringTransactionsMenu(chatId);
      } else {
        await showMainMenu(chatId);
      }
...
async function handleUserInput(chatId, userId, text, session) {
  ...
  } else if (session.data && session.data.isRecurring) {
    await handleRecurringUserInput(chatId, userId, text, session);
