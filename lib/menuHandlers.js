...
export async function showCategorySelection(chatId, transactionType, month) {
  ...
  // Create buttons for each category with explanations
  const categoryDescriptions = {
    'Survival': '🟢 Survival\nFood, Hygiene, Kids, Transport',
    'Optional': '🟡 Optional\nClothes, Skin Care, Snacks, Cafe & Resto, Vacation, Gifts',
    'Culture': '🔵 Culture\nBooks, Films, Course, Music',
    'Extra': '🔴 Extra\nHospital, House Renov, Electronic Broken, Someone\'s Loan'
  };

  CONFIG.CATEGORIES.forEach(category => {
    keyboard.inline_keyboard.push([{
      text: categoryDescriptions[category] || category,
  ...
  await sendTelegramMessage(chatId, "📂 Select Category:", keyboard);
}
