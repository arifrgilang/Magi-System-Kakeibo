...
const RECURRING_TRANSACTIONS = {
  ...
  'motor_gas': {
    ...
    emoji: '⛽'
    // No predefined description - user will input provider name
  }
...
export async function handleRecurringTransactionSelection(chatId, userId, transactionKey) {
  ...
  if (transaction.type === 'variable') {
    ...
    // Only add description if it exists (some fixed expenses like Motor Gas don't have predefined descriptions)
    if (transaction.description) {
      sessionData.description = transaction.description;
    }
    ...
    console.log('Set up fixed expense session');
    if (transaction.description) {
      console.log('With predefined description:', transaction.description);
    }
    if (transaction.preselectedAccount) {
      console.log('With preselected account:', transaction.preselectedAccount);
    }
  }
...
async function handleRecurringAmountInput(chatId, userId, text, session) {
  ...
  if (updatedSession.type === 'fixed' && updatedSession.data && updatedSession.data.description) {
    // Process immediately for fixed expenses with predefined description (Pasar/Supa Groceries & Amenities)
    await processRecurringTransaction(chatId, userId, updatedSession);
  } else {
    // Ask for description for variable expenses or fixed expenses without predefined description (Motor Gas)
    SessionManager.updateSession(userId, { 
      step: 'description', 
      awaitingInput: true 
    });
