...
async function handleRecurringAmountInput(chatId, userId, text, session) {
  ...
  // TEMPORARY: Send debug info to Telegram
  await sendTelegramMessage(chatId, 
    `🐛 DEBUG INFO:\n` +
    `Type: ${updatedSession.type}\n` +
    `Recurring: ${updatedSession.data?.recurringType}\n` +
    `Has Description: ${!!updatedSession.data?.description}\n` +
    `Description: "${updatedSession.data?.description}"`
  );
  ...
  if (updatedSession.type === 'fixed' && updatedSession.data && updatedSession.data.description) {
    ...
    await sendTelegramMessage(chatId, "✅ Fixed expense detected - processing immediately...");
    ...
    await sendTelegramMessage(chatId, "❌ Variable expense or missing description - asking for input...");
