...
async function handleRecurringAmountInput(chatId, userId, text, session) {
  ...
  console.log('Session data after amount input:', session.data);
  console.log('Has predefined description:', !!session.data?.description);
  
  // Check if description is already provided (for fixed expenses)
  if (session.data && session.data.description) {
    console.log('Processing fixed expense with predefined description:', session.data.description);
