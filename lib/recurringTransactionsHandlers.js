...
async function handleRecurringAmountInput(chatId, userId, text, session) {
  ...
  // Get fresh session data after updating amount
  const updatedSession = SessionManager.getSession(userId);
  console.log('Session data after amount input:', updatedSession.data);
  console.log('Has predefined description:', !!updatedSession.data?.description);
  
  // Check if description is already provided (for fixed expenses)
  if (updatedSession.data && updatedSession.data.description) {
    console.log('Processing fixed expense with predefined description:', updatedSession.data.description);
    ...
    console.log('Asking for description (variable expense)');
